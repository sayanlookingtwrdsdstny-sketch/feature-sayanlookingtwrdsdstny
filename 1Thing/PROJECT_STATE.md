...
1. **Local dev environment quirk (fixed):** `local.properties` had a UTF-8 BOM that broke `sdk.dir` parsing, and the original `sdk.dir` value used backslash-escaped syntax that Gradle mis-parsed on this machine. Rewritten as `sdk.dir=C:/Users/Sayan` (forward slashes, no BOM) — if this file gets regenerated by Android Studio and builds start failing with "SDK location not found", check for the BOM/escaping issue again.
